...
export default function Sleep() {
  const [playingSounds, setPlayingSounds] = useState<Record<string, { howl: Howl, volume: number }>>({});
  ...
  const toggleSound = (sound: typeof SOUND_MODES[0]) => {
    if (playingSounds[sound.id]) {
      playingSounds[sound.id].howl.stop();
      const newSounds = { ...playingSounds };
      delete newSounds[sound.id];
      setPlayingSounds(newSounds);
    } else {
      const howl = new Howl({
        src: [sound.url],
        loop: true,
      ...
      howl.play();
      setPlayingSounds({ ...playingSounds, [sound.id]: { howl, volume: 0.5 } });
    }
  };

  const updateVolume = (id: string, volume: number) => {
    if (playingSounds[id]) {
      playingSounds[id].howl.volume(volume);
      setPlayingSounds({ 
        ...playingSounds, 
        [id]: { ...playingSounds[id], volume } 
      });
    }
  };

  const stopAllSounds = () => {
    (Object.values(playingSounds) as { howl: Howl; volume: number }[]).forEach(s => s.howl.stop());
    setPlayingSounds({});
  };
  ...
  return (
    <div className="w-full max-w-md mx-auto space-y-8 pb-20">
      <header className="flex items-center justify-between">
        ...
        <h2 className="text-xl font-medium">Modo Sueño</h2>
        <div className="w-10" />
      </header>
      ...
      <section className="text-center space-y-2">
        ...
        <p className="text-white/40 text-sm">Mezcla sonidos para tu descanso ideal</p>
      </section>
      ...
      <GlassCard className="p-6">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
          ...
          {sleepTimer && (
            <button 
              onClick={() => { setSleepTimer(null); setTimeLeft(null); }}
                ...
                {isPlaying && (
                  <div className="flex items-center gap-3">
                    ...
                    <input 
                      ...
                      max="1" 
                      step="0.01" 
                      value={playingSounds[sound.id]?.volume || 0.5}
